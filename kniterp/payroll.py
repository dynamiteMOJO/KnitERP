...
def calculate_variable_pay(slip, method):
    ...
    conveyance = get_conveyance(employee, start)
...
def get_per_day_salary(slip):
    ...
    tea = get_component_amount(slip, "Tea Allowance")
...
def get_conveyance(employee, month_start):
...
def get_rejected_holiday_days(employee, start, end):
    ...
    for h in holidays:
        ...
        if is_leave(employee, prev_day) and is_leave(employee, next_day):
            rejected += 1
