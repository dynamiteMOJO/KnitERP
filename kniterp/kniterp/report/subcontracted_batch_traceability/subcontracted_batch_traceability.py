...
def execute(filters: dict | None = None):
	...
	if not report.validate_filters():
		# Return empty columns and data if validation fails
		return [], []
...
class ReportData:
	...
	def validate_filters(self):
		if not self.filters or (not self.filters.get("item_code") and not self.filters.get("batches") and not self.filters.get("serial_nos")):
			return False
		return True
	...
	def get_data(self):
		...
		if self.filters.get("traceability_direction") in ["Backward", "Both"]:
			...
			for key in source_data:
				...
				if sabb_data.reference_doctype not in ["Stock Entry", "Subcontracting Receipt"]:
					continue
	...
	def get_serial_no_batches(self, name=None):
		batches = self.filters.get("batches", []) if self.filters else []
		serial_nos = self.filters.get("serial_nos", []) if self.filters else []
		...
		if self.filters and self.filters.get("item_code"):
			if self.doctype_name == "Serial No":
				query = query.where(doctype.item_code == self.filters.get("item_code"))
			else:
				query = query.where(doctype.item == self.filters.get("item_code"))
	...
	def get_doctype(self):
		...
		elif self.filters and self.filters.get("serial_nos"):
			return "Serial No"
	...
	def get_materials_for_subcontracting_receipt(self, sabb_data):
		scr = frappe.qb.DocType("Subcontracting Receipt")
		scr_item = frappe.qb.DocType("Subcontracting Receipt Supplied Item")
		sabb_entry = frappe.qb.DocType("Serial and Batch Entry")

		query = (
			frappe.qb.from_(scr)
			.inner_join(scr_item)
			.on(scr.name == scr_item.parent)
			.left_join(sabb_entry)
			.on(
				(scr_item.serial_and_batch_bundle == sabb_entry.parent)
				& (sabb_entry.docstatus == 1)
			)
			.select(
				sabb_entry.warehouse.as_("warehouse"),
				scr_item.rm_item_code.as_("item_code"),
				scr_item.name,
				scr_item.item_name.as_("item_name"),
				scr_item.parenttype.as_("reference_doctype"),
				scr.name.as_("reference_name"),
				(
					(
						scr_item.consumed_qty
						/ Case()
						.when(scr.total_qty > 0, scr.total_qty)
						.else_(sabb_data.qty)
					)
					* sabb_data.qty
				).as_("qty"),
			...
			.where(
				(scr.docstatus == 1)
				& (scr.name == sabb_data.reference_name)
			)
		)
	...
	def get_materials(self, sabb_data):
		if sabb_data.reference_doctype == "Subcontracting Receipt":
			return self.get_materials_for_subcontracting_receipt(sabb_data)
	...
	def set_forward_data(self, value, sabb_data):
		...
		for row in outward_entries:
			...
			elif row.reference_doctype == "Subcontracting Receipt":
				self.process_subcontracting_receipt_entry(row, sabb_data)
			else:
	...
	def process_subcontracting_receipt_entry(self, row, batch_details):
		fg_item = self.get_finished_item_from_subcontracting_receipt(row.reference_name)
		if not fg_item:
			return
	...
	def get_finished_item_from_subcontracting_receipt(self, reference_name):
	...
	def get_columns(self, has_serial_no=None, has_batch_no=None):
		...
		if self.filters and self.filters.get("traceability_direction") == "Backward":
			columns.append(
				{
					"fieldname": "supplier",
					"label": _("Supplier"),
